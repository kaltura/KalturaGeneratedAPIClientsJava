...
public class KalturaEmailNotificationCategoryRecipientJobData extends KalturaEmailNotificationRecipientJobData {
    ...
    public KalturaEmailNotificationCategoryRecipientJobData(Element node) throws KalturaApiException {
        super(node);
        NodeList childNodes = node.getChildNodes();
        for (int i = 0; i < childNodes.getLength(); i++) {
            Node aNode = childNodes.item(i);
            String nodeName = aNode.getNodeName();
            if (nodeName.equals("categoryUserFilter")) {
                this.categoryUserFilter = ParseUtils.parseObject(KalturaCategoryUserFilter.class, aNode);
                continue;
            } 
        }
    }
