...
public class KalturaOrCondition extends KalturaCondition {
    public ArrayList<KalturaCondition> conditions;

    public KalturaOrCondition() {
    }

    public KalturaOrCondition(Element node) throws KalturaApiException {
        super(node);
        NodeList childNodes = node.getChildNodes();
        for (int i = 0; i < childNodes.getLength(); i++) {
            Node aNode = childNodes.item(i);
            String nodeName = aNode.getNodeName();
            if (nodeName.equals("conditions")) {
