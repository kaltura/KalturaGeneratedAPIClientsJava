...
public class KalturaEmailNotificationStaticRecipientProvider extends KalturaEmailNotificationRecipientProvider {
    ...
    public KalturaEmailNotificationStaticRecipientProvider(Element node) throws KalturaApiException {
        super(node);
        NodeList childNodes = node.getChildNodes();
        for (int i = 0; i < childNodes.getLength(); i++) {
            Node aNode = childNodes.item(i);
            String nodeName = aNode.getNodeName();
            if (nodeName.equals("emailRecipients")) {
                this.emailRecipients = ParseUtils.parseArray(KalturaEmailNotificationRecipient.class, aNode);
                continue;
            } 
        }
    }
