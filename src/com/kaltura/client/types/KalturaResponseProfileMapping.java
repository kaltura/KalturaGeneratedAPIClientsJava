...
public class KalturaResponseProfileMapping extends KalturaObjectBase {
    ...
    public KalturaResponseProfileMapping(Element node) throws KalturaApiException {
        NodeList childNodes = node.getChildNodes();
        for (int i = 0; i < childNodes.getLength(); i++) {
            Node aNode = childNodes.item(i);
            String nodeName = aNode.getNodeName();
            String txt = aNode.getTextContent();
            if (nodeName.equals("parentProperty")) {
                this.parentProperty = ParseUtils.parseString(txt);
                continue;
