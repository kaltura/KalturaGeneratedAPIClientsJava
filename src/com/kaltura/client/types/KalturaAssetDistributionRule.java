...
public class KalturaAssetDistributionRule extends KalturaObjectBase {
    ...
    public KalturaAssetDistributionRule(Element node) throws KalturaApiException {
        NodeList childNodes = node.getChildNodes();
        for (int i = 0; i < childNodes.getLength(); i++) {
            Node aNode = childNodes.item(i);
            String nodeName = aNode.getNodeName();
            String txt = aNode.getTextContent();
            if (nodeName.equals("validationError")) {
                this.validationError = ParseUtils.parseString(txt);
                continue;
            } else if (nodeName.equals("assetDistributionConditions")) {
                this.assetDistributionConditions = ParseUtils.parseArray(KalturaAssetDistributionCondition.class, aNode);
                continue;
            } 
        }
    }
