...
public enum MediaEntryOrderBy implements EnumAsString {
	...
	public static MediaEntryOrderBy get(String value) {
		if(value == null)
		{
			return null;
		}
		
		// goes over MediaEntryOrderBy defined values and compare the inner value with the given one:
		for(MediaEntryOrderBy item: values()) {
			if(item.getValue().equals(value)) {
				return item;
			}
		}
		// in case the requested value was not found in the enum values, we return the first item as default.
		return MediaEntryOrderBy.values().length > 0 ? MediaEntryOrderBy.values()[0]: null;
   }
}
