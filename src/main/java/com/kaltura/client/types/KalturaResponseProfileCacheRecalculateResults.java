...
public class KalturaResponseProfileCacheRecalculateResults extends KalturaObjectBase {
    ...
    public KalturaResponseProfileCacheRecalculateResults(Element node) throws KalturaApiException {
        NodeList childNodes = node.getChildNodes();
        for (int i = 0; i < childNodes.getLength(); i++) {
            Node aNode = childNodes.item(i);
            String nodeName = aNode.getNodeName();
            String txt = aNode.getTextContent();
            if (nodeName.equals("lastObjectKey")) {
                this.lastObjectKey = ParseUtils.parseString(txt);
                continue;
            } else if (nodeName.equals("recalculated")) {
                this.recalculated = ParseUtils.parseInt(txt);
                continue;
            } 
        }
    }
