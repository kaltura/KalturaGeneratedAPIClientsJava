...
@MultiRequestBuilder.Tokenizer(ESearchCategoryUserItem.Tokenizer.class)
public class ESearchCategoryUserItem extends ESearchAbstractCategoryItem {
	...
	public void setFieldName(ESearchCategoryUserFieldName fieldName){
	...
	public void permissionName(String multirequestToken){
		setToken("permissionName", multirequestToken);
	}
	...
	public ESearchCategoryUserItem(JsonObject jsonObject) throws APIException {
		super(jsonObject);

		if(jsonObject == null) return;

		// set members values:
		fieldName = ESearchCategoryUserFieldName.get(GsonParser.parseString(jsonObject.get("fieldName")));
		permissionLevel = CategoryUserPermissionLevel.get(GsonParser.parseInt(jsonObject.get("permissionLevel")));
		permissionName = GsonParser.parseString(jsonObject.get("permissionName"));

	}

	public Params toParams() {
		Params kparams = super.toParams();
		kparams.add("objectType", "KalturaESearchCategoryUserItem");
		kparams.add("fieldName", this.fieldName);
		kparams.add("permissionLevel", this.permissionLevel);
		kparams.add("permissionName", this.permissionName);
		return kparams;
	}
