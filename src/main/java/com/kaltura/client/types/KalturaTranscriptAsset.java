...
public class KalturaTranscriptAsset extends KalturaAttachmentAsset {
    ...
    public KalturaTranscriptAsset(Element node) throws KalturaApiException {
        super(node);
        ...
        for (int i = 0; i < childNodes.getLength(); i++) {
            ...
            if (nodeName.equals("accuracy")) {
                this.accuracy = ParseUtils.parseDouble(txt);
                continue;
            } else if (nodeName.equals("humanVerified")) {
                this.humanVerified = KalturaNullableBoolean.get(ParseUtils.parseInt(txt));
                continue;
