...
public class KalturaFileSyncDescriptor extends KalturaObjectBase {
    ...
    public KalturaFileSyncDescriptor(Element node) throws KalturaApiException {
        NodeList childNodes = node.getChildNodes();
        for (int i = 0; i < childNodes.getLength(); i++) {
            Node aNode = childNodes.item(i);
            String nodeName = aNode.getNodeName();
            String txt = aNode.getTextContent();
            if (nodeName.equals("fileSyncLocalPath")) {
                this.fileSyncLocalPath = ParseUtils.parseString(txt);
                continue;
            } else if (nodeName.equals("fileSyncRemoteUrl")) {
                this.fileSyncRemoteUrl = ParseUtils.parseString(txt);
                continue;
            } else if (nodeName.equals("fileSyncObjectSubType")) {
                this.fileSyncObjectSubType = ParseUtils.parseInt(txt);
                continue;
            } 
        }
    }
