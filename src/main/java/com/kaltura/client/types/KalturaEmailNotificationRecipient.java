...
public class KalturaEmailNotificationRecipient extends KalturaObjectBase {
    ...
    public KalturaEmailNotificationRecipient(Element node) throws KalturaApiException {
        NodeList childNodes = node.getChildNodes();
        for (int i = 0; i < childNodes.getLength(); i++) {
            Node aNode = childNodes.item(i);
            String nodeName = aNode.getNodeName();
            if (nodeName.equals("email")) {
                this.email = ParseUtils.parseObject(KalturaStringValue.class, aNode);
                continue;
            } else if (nodeName.equals("name")) {
                this.name = ParseUtils.parseObject(KalturaStringValue.class, aNode);
                continue;
            } 
        }
    }
