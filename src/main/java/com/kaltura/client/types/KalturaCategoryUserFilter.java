...
public class KalturaCategoryUserFilter extends KalturaCategoryUserBaseFilter {
    ...
    public KalturaCategoryUserFilter(Element node) throws KalturaApiException {
        super(node);
        ...
        for (int i = 0; i < childNodes.getLength(); i++) {
            ...
            if (nodeName.equals("categoryDirectMembers")) {
                this.categoryDirectMembers = ParseUtils.parseBool(txt);
                continue;
            } else if (nodeName.equals("freeText")) {
                this.freeText = ParseUtils.parseString(txt);
                continue;
            } else if (nodeName.equals("relatedGroupsByUserId")) {
                this.relatedGroupsByUserId = ParseUtils.parseString(txt);
                continue;
            } 
        }
    }
