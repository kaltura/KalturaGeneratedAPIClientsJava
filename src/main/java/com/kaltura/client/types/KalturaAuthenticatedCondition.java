...
public class KalturaAuthenticatedCondition extends KalturaCondition {
    ...
    public KalturaAuthenticatedCondition(Element node) throws KalturaApiException {
        super(node);
        NodeList childNodes = node.getChildNodes();
        for (int i = 0; i < childNodes.getLength(); i++) {
            Node aNode = childNodes.item(i);
            String nodeName = aNode.getNodeName();
            if (nodeName.equals("privileges")) {
                this.privileges = ParseUtils.parseArray(KalturaStringValue.class, aNode);
                continue;
            } 
        }
    }
