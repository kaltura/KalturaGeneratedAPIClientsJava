...
import com.kaltura.client.enums.KalturaEmailNotificationRecipientProviderType;
...
public abstract class KalturaEmailNotificationRecipientJobData extends KalturaObjectBase {
    ...
    public KalturaEmailNotificationRecipientJobData(Element node) throws KalturaApiException {
        NodeList childNodes = node.getChildNodes();
        for (int i = 0; i < childNodes.getLength(); i++) {
            Node aNode = childNodes.item(i);
            String nodeName = aNode.getNodeName();
            String txt = aNode.getTextContent();
            if (nodeName.equals("providerType")) {
                this.providerType = KalturaEmailNotificationRecipientProviderType.get(ParseUtils.parseString(txt));
                continue;
            } 
        }
    }
